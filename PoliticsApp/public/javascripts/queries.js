const queries = {
	...
	get_member_of_committee_in_closest_race : {
		...
		query : 
			`SELECT DISTINCT m.firstname, m.lastname, p.win_probability as winProbability 
			FROM
              (SELECT * FROM CommitteeAssignment WHERE committee_id = {}) ca 
            JOIN Member m ON ca.state = m.state AND ca.district = m.district 
            JOIN PollLite p ON m.state = p.state AND m.district = p.district WHERE
              p.is_incumbent = 1 AND p.win_probability + 50 =
              (SELECT MIN(ABS(p.win_probability - 50)) FROM (SELECT * FROM
              CommitteeAssignment WHERE committee_id = {}) ca JOIN Member m
              ...
              WHERE committee_id = {}) ca JOIN Member m ON ca.state = m.state AND
